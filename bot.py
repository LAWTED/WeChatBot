...
class AsyncHTTPHandler(logging.Handler):
    ...
    def _process_queue(self):
        headers = {"Content-Type": "application/json", "User-Agent": "WeChatBot/1.0"}
...
def parse_time(time_str):
    try:
        ...
        logger.error(
            "\033[31m错误：主动消息安静时间设置有误！请填00:00-23:59 不要填24:00,并请注意中间的符号为英文冒号！\033[0m"
        )
...
def get_deepseek_response(message, user_id):
    try:
        ...
        if "real name verification" in ErrorImformation:
            ...
            logger.error(
                "\033[31m错误：API服务商反馈当前访问API服务频次达到上限，请稍后再试！ \033[0m"
            )
        elif "paid" in ErrorImformation:
            logger.error(
                "\033[31m错误：API服务商反馈您正在使用付费模型，请先充值再使用或使用免费额度模型！ \033[0m"
            )
        elif "Api key is invalid" in ErrorImformation:
            logger.error(
                "\033[31m错误：API服务商反馈API KEY不可用，请检查配置选项！ \033[0m"
            )
                ...
                for msg in one_msgs:
                    ...
                    if who == msg.sender:
                        ...
                        if "[动画表情]" in content and ENABLE_EMOJI_RECOGNITION:
                            ...
                            handle_wxauto_message(msg, who)
                    elif ROBOT_WX_NAME != "" and (
                        bool(re.search(f"@{ROBOT_WX_NAME}\u2005", msg.content))
                    ):
                        # 处理群聊信息，只有@当前机器人才会处理
                        msg.content = re.sub(
                            f"@{ROBOT_WX_NAME}\u2005", "", content
                        ).strip()
                        msg.content = "群聊消息[" + msg.sender + "]:" + msg.content
                        handle_wxauto_message(msg, who)
                    else:
...
def recognize_image_with_moonshot(image_path, is_emoji=False):
    ...
    text_prompt = (
        "请描述这个图片" if not is_emoji else "请描述这个聊天窗口的最后一张表情包"
    )
    data = {
    ...
    try:
        response = requests.post(
            f"{MOONSHOT_BASE_URL}/chat/completions", headers=headers, json=data
        )
        ...
        if is_emoji:
            ...
            if "最后一张表情包是" in recognized_text:
                recognized_text = recognized_text.split("最后一张表情包是", 1)[
                    1
                ].strip()
...
def handle_wxauto_message(msg, who):
    try:
        ...
        # 检查是否是图片消息
        if content and content.lower().endswith(
            (".png", ".jpg", ".jpeg", ".gif", ".bmp")
        ):
        ...
        if img_path:
            ...
            content = (
                recognized_text if content is None else f"{content} {recognized_text}"
            )
            # 清理临时文件
            clean_up_temp_files()
            can_send_messages = True
        ...
        if content:
            ...
            if ENABLE_MEMORY:
                ...
                prompt_name = prompt_mapping.get(
                    username, username
                )  # 获取配置的prompt名
                log_file = os.path.join(
                    root_dir, MEMORY_TEMP_DIR, f"{username}_{prompt_name}_log.txt"
                )
                ...
                if (
                    ...
                    and os.path.getsize(log_file) > 1 * 1024 * 1024
                ):  # 1MB
                    archive_file = os.path.join(
                        root_dir,
                        MEMORY_TEMP_DIR,
                        f"{username}_log_archive_{int(time.time())}.txt",
                    )
    ...
    while True:
        ...
        with queue_lock:
            for username, user_data in user_queues.items():
                ...
                if (
                    current_time - last_time > QUEUE_WAITING_TIME
                    and can_send_messages
                    and not is_sending_message
                ):
            ...
            for i, part in enumerate(parts):
                ...
                if i < len(parts) - 1:
                    ...
                    delay = len(next_part) * (
                        ...
                        + random.uniform(
                            RANDOM_TYPING_SPEED_MIN, RANDOM_TYPING_SPEED_MAX
                        )
                    )
                    if delay < 2:
                        delay = 2
...
def capture_and_save_screenshot(who):
    ...
    screenshot_path = os.path.join(
        screenshot_folder, f'{who}_{datetime.now().strftime("%Y%m%d%H%M%S")}.png'
    )
...
def append_to_memory_section(user_id, content):
    ...
    try:
        ...
        with open(user_file, "r+", encoding="utf-8") as file:
            ...
            memory_marker = "开始更新："
            insert_index = next(
                (i for i, line in enumerate(lines) if memory_marker in line), -1
            )
...
def summarize_and_save(user_id):
    ...
    try:
        ...
        log_file = os.path.join(
            root_dir, MEMORY_TEMP_DIR, f"{user_id}_{prompt_name}_log.txt"
        )
        ...
        if importance_match:
            ...
            importance = 3  # 默认值
            logger.warning(
                f"无法解析重要性评分，使用默认值3。原始响应：{importance_response}"
            )
    ...
    while True:
        try:
            ...
            for user in user_names:
                ...
                log_file = os.path.join(
                    root_dir, MEMORY_TEMP_DIR, f"{user}_{prompt_name}_log.txt"
                )
                ...
                try:
                    ...
                    user_prompt_file = os.path.join(
                        root_dir, "prompts", f"{prompt_name}.md"
                    )
...
def manage_memory_capacity(user_file):
    ...
    try:
        ...
        sorted_indices = sorted(
            range(len(memory_scores)), key=lambda k: (-memory_scores[k], segments[k][0])
        )
        ...
        for user in user_names:
            ...
            if not os.path.exists(prompt_path):
                raise FileNotFoundError(
                    f"用户 {user} 的prompt文件 {prompt_file}.md 不存在"
                )
...
if __name__ == "__main__":
    try:
        ...
        logger.error(
            f"FALLBACK LOG: {datetime.now()} - ERROR - {str(e)}"
        )  # 保证基础日志可见
