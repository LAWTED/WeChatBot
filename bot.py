...
def get_alice_response(message, user_id):
    try:
        logger.info(f"调用 Alice API - 用户ID: {user_id}, 消息: {message}")
        ...
        with queue_lock:
            ...
            # 获取所有对话记录，不再限制只取最近10条
            conversationHistory = []
            for msg in chat_contexts[user_id]:  # 获取全部历史消息
        ...
        request_data = {
            ...
            "userid": user_id,
            "model": "gpt-4.1-2025-04-14"  # 可以在配置中设置
        }

        # 发送请求到 Alice API
        response = requests.post(
            'https://hai.lawted.tech/api/alice',
            headers=headers,
            json=request_data,
            timeout=60
        )
        ...
        if not result.get('success', False):
            logger.error(f"Alice API 返回错误: {result.get('message', 'Unknown error')}")
            return "服务响应异常，请稍后再试"
        ...
        logger.info(f"Alice API回复: {reply}")
        return reply

    except requests.exceptions.RequestException as e:
        logger.error(f"Alice API请求失败: {str(e)}", exc_info=True)
        return "抱歉，我现在无法连接到服务器，请稍后再试。"

    except Exception as e:
        error_info = str(e)
        logger.error(f"调用失败: {error_info}", exc_info=True)

        # 特定错误信息的处理
        if "404" in error_info:
            logger.error("\033[31m错误：Alice API地址无效或服务未启动！\033[0m")
        elif "timeout" in error_info.lower():
            logger.error("\033[31m错误：请求超时，服务器可能繁忙！\033[0m")
        else:
            logger.error(f"\033[31m错误：{error_info}\033[0m")
...
def process_user_messages(user_id):
    ...
    # 获取 API 回复
    reply = get_alice_response(merged_message, user_id)
